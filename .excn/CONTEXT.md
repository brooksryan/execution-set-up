...
Format authority: `.claude/skills/execution-context-grill/CONTEXT-FORMAT.md`.
...
### Setup Skill
The lightweight skill that is the framework's sole unit of distribution — you get the skill, and it bootstraps everything else. Invoked by the Team Lead in a new project ("use the execution-set-up repo"), it runs four phases: **Preflight** (hard gate: requires Node 18+; no global skills required — the Scaffolder stamps what setup uses and what the later grill family needs) → run the **Scaffolder** to stamp the invariant layout → **Grill** → the agent writes only the variant files into the stamped tree → **Handoff**. It reads lightly and defers to the agent's judgment rather than prescribing every step; it carries its own Setup Grill interview — the Grill phase runs from the skill file itself, no stamped skill invoked. Fresh install today is the npm path: `npm install to-execution`, then the agent reads and follows `node_modules/to-execution/SKILL.md` — the skill is written for that reader, assuming no skill registry and no prior context; registry and plugin invocations read the same file. Official skill/plugin-surface publication is tracked as EXEC-085.
...
### Grill Family
Three grills, each keyed by the artifact it owns (ADR-0009) — "which grill am I in?" is answered by "what am I allowed to write?" The **Setup Grill** is the one-time Setup Skill interview (scaffold + context/philosophy/team seeds) that runs inside the Setup Skill end-to-end and ends at Handoff without planning work. The **Context Grill** (`execution-context-grill`, recurring) establishes or extends the workspace's shared model: writes terms into CONTEXT.md term-by-term, may seed PHILOSOPHY.md only while the stamped sentinel is present (ADR-0010), then the Retro Loop owns it. The **Epic Grill** (`execution-epic-grill`, recurring, renamed from `execution-grill-with-docs` in ADR-0009) grills a body of work into requirements feeding `execution-to-prd`, with an optional deep design pass (work-in-types) the agent offers when design-heavy or on request. Writes design decisions to ADRs, never to CONTEXT.md or PHILOSOPHY.md. The Lifecycle's grill node: context-grill (when shared model has shifted) → epic-grill → PRD → issues. Each grill runs in a fresh session; context-grill's CONTEXT.md writes must be committed before epic-grill reads them.
_Avoid_: "Work Grill" unqualified — name the specific grill (Context Grill or Epic Grill). "execution-grill-with-docs" — the old name, retired in the rename (ADR-0009); use "execution-epic-grill".
...
- The **Team Lead** drives the **Lifecycle** (context-grill → epic-grill → PRD → issues → sprint → retro); the **Retro Loop** is the only edge from a sprint's retro back into **persistent docs / Teammate definitions**, gated by the **alignment** Adherence Agent and approved by the Team Lead.
...
- **"Grill"** — ambiguous among the one-time **Setup Grill**, the recurring **Context Grill**, and the recurring **Epic Grill**. Resolution: always name the specific grill; they run in separate sessions, each owns one artifact (ADR-0009).
