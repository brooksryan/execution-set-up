...
Two hard-separated grills. The **Setup Grill** is the one-time Setup Skill interview (context + philosophy + team) that ends at Handoff and does **not** plan work. The **Work Grill** is the recurring Lifecycle entry point (`grill → PRD → issues`) run in a *fresh session* after setup to start a sprint against the current goal. Separation keeps setup context out of work-planning context.
