...
2. Check every rule:
   ...
   - **Gitignore** — `.excn/.gitignore` is stamped containing the work-tracking ignore set (`sprints/`, `issues/`, `prds/`, `retros/`, `*_progress.json`); the host root `.gitignore` is NEVER touched by init; every pattern in the stamped set matches only at its intended depth (anchoring check).
