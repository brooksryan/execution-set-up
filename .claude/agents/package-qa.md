...
2. Check every rule:
   ...
   - **Gitignore** — `.excn/.gitignore` is stamped containing `*_progress.json` plus the pre-warm Runtime Record patterns `runtime/grounding-pack.json`, `runtime/prd-draft.json`, `runtime/grill-decisions.log`; the host root `.gitignore` is NEVER touched by init; the patterns match only inside `.excn/` (anchoring check), and each pre-warm pattern is anchored to `runtime/` (never a bare glob).
