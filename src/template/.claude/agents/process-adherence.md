...
2. Evaluate the transition against every rule. Key checks:
   ...
   - **Sprint completion** — if closing a sprint: read the sprint record and its companion partition directory `.excn/issues/sprint-<N>/` (the per-file issue records); no item is still `in_progress`, decisions and `retrospective_notes` are recorded, and every mandatory QA gate passed.
