...
How work moves in this project. This document is invariant — the same in every project set up with this framework. It is the rubric the `process-adherence` Adherence Agent enforces. Project-specific rules live in `TEAM_DIRECTIVE.md`; the domain glossary lives in `CONTEXT.md`; the universal Principles are baked into the framework.
...
Each sprint is one JSON file at `tmp/exec/sprints/sprint_<N>.json`, conforming to `schemas/sprint.schema.json`. scribe owns it. The JSON is the source of truth.
...
3. scribe spawns the `alignment` agent with the proposed change + Principles + `PHILOSOPHY.md`.
...
Adherence Agents gate work. Two are universal: `process-adherence` (this document) and `alignment` (Principles + Philosophy). The Team Lead authors one-off agents for project-specific rubrics (see `TEAM_DIRECTIVE.md` for which, and the framework's `authoring-adherence-agents.md` for how).

Gates are **mandatory for sprint-significant artifacts** and recommended below that. A `FAIL` sends the author back to revise, then back through the gates — never forward. `TEAM_DIRECTIVE.md` declares exactly which work triggers which gate.
